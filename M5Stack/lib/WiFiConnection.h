...
void emptyWiFiStatusMessage ()
{
}

struct WiFiStatusMessages wifiStatusMessages = {
	emptyWiFiStatusMessage,
	emptyWiFiStatusMessage,
	emptyWiFiStatusMessage,
	emptyWiFiStatusMessage,
	emptyWiFiStatusMessage,
...
void start_wifi_scan()
{
	...
	wifiStatusMessages.display_starting_wifi_scan();
...
void loop_wifi()
{
	...
	case WiFiScanning:
		...
		for (int i = 0; i < no_of_networks; ++i) {
			...
			if (connectionNumber != WIFI_SETTING_NOT_FOUND)
			{
				...
				update_action(wiFiSettings.wifiConnections[connectionNumber].wifiSsid,
					"Connecting");
				wifiStatusMessages.display_connecting_to_wifi();
		...
		if (wifiState != WiFiConnecting)
		{
			...
			wifiStatusMessages.display_no_wifi_networks_found();
		}
	...
	case WiFiConnecting:
		...
		if (WiFi.status() == WL_CONNECTED)
		{
			...
			wifi_timer_start = millis();
			wifiStatusMessages.display_wifi_connected_ok();
		}
		...
		if (elapsed_time > WIFI_START_TIMEOUT)
		{
			...
			wifiState = WiFiConnectFailed;
			wifiStatusMessages.display_wifi_connect_failed();
		}
	...
	case WiFiConnected:
		...
		if (WiFi.status() != WL_CONNECTED)
		{
			...
			wifiStatusMessages.display_wifi_failed_in_use();
		}
		break;
